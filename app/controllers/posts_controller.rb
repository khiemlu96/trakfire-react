...
class PostsController < ApplicationController
	...
	def create
	  ...
	  if @post.save
	    ...
	    render json: @post, status: :created #, include: { user: { only: [:handle, :id, :username, :tbio, :img, :isAdmin, :canPost] } }, only: [:id, :title, :stream_url, :duration, :artist, :img_url, :img_url_lg, :created_at, :duration, :genre, :vote_count], status: :created, location: post_url(@post, format: :json)
