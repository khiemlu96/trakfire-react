class FollowerController < ApplicationController
	...
	def create
		@follower = Follower.new(follower_params)
	  	...
	  	existing_follow = Follower.where(follow_id: follower_params[:follow_id], user_id: @current_user.id)

	  	if existing_follow
	  		if @current_user.id != @follower.follow_id
				if @follower.save
					@notification = {
			...
			render json: @follower.errors, status: :duplicate_entry
	  	end	  	
	end	
